...
import com.ap88.yg.fruittole.domain.model.ProductTag;
...
public class FlowLayout extends RelativeLayout {
  ...
  // 关键字背景
  private int backgroundResource = R.drawable.drawable_tag_apple_info;
  // 关键字水平padding，单位为dp
  ...
  public FlowLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (attr == R.styleable.FlowLayoutAttrs_horizontalSpacing) {
        ...
        backgroundResource = typedArray.getResourceId(attr, R.drawable.drawable_tag_apple_info);
  ...
  public void setFlowLayoutByType(List<ProductTag> list) {
    for (int i = 0; i < list.size(); i++) {
      final TextView tv = new TextView(getContext());

      // 设置TextView属性
      tv.setText(list.get(i).getTagName());
      tv.setTextSize(TypedValue.COMPLEX_UNIT_PX, textSize);
      tv.setTextColor(Color.parseColor(list.get(i).getTagTextColor()));

      tv.setBackgroundResource(list.get(i).getTagBgRes());
      tv.setGravity(Gravity.CENTER);
      tv.setPadding(textPaddingH, textPaddingV, textPaddingH, textPaddingV);
